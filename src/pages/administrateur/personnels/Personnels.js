...
const Personnels = () =>{
 ...
 /*PERSONNEL*/
    ...
    const reccord = personnels.slice(firstindex, lastindex);
    const nbrePage = Math.ceil(personnels.length/reccordPage);
    const number = [...Array(nbrePage + 1).keys()].slice(1);
    /*********************** */

    /*Fonction qui affiche les personnels*/
    useEffect(()=>{Personnel.AfficherPersonnel(setPersonnels)},[])
    /************** **************/
    ...
    /***************************/
    /*fonction qui retire un membre du personnel*/
    const Supprimer = async (id) =>{
        await axios.post(Config.url+Config.port8081+`/public/supprimer/${id}`).then(() => {
            // Mettez à jour l'état local (supprimez l'employé de la liste)
            navigate("/personnels")
        })
        .catch((error) => {
                ...
                <main class="app-main"> 
                    <div class="app-content-header"> 
                        <div class="row">
                            ...
                            <div class="col-md-4">
                                <h1 class="h1"><b>Liste du personnel</b></h1>
                            </div>
                            <div class="col-md-4"></div>
                        </div><br/>
                                ...
                                <div class="col-md-10">
                                    <div class="card card-success">
                                        <div class="card-header">
                                            <h3 class="card-title"><strong>Membres du personnel</strong></h3>
                                        </div> 
                                        <div class="card-body p-3">
                                            <div class="table-responsive">
                                                <table class="table table-default table-striped">
                                                    ...
                                                    <tbody>
                                                        {reccord.map((personnel,i) => (
                                                            <tr key={i}>
                                                                <td>{personnel.name}</td>
                                                                <td>{personnel.prename}</td>
                                                                <td>{personnel.addressmail}</td>
                                                                <td>{personnel.telephone}</td>
                                                                <td>{personnel.role}</td>
                                                                <td class="mailbox-date">
                                                                    <Link className="btn btn-success"><i class="bi bi-pencil-fill"></i></Link>{" "}
                                                                    <Link className="btn btn-danger" onClick={() =>{Supprimer(personnel.id)}}><i class="bi bi-trash3-fill"></i></Link>
                                                                </td>
                                                            </tr>
                                                        ))}
                                                    </tbody>
